...
Throughput = c("30,000–50,000 probes/array", "40,000–60,000 probes/array", ">48,000 probes/array"),
Average_Price = c("~$250–$400 per array", "~$200–$300 per array", "~$150–$250 per array")
)
...
2. **Core analysis logic**  
   - Quality assessment → alignment → quantification
...
- **Main steps:**  
  ...
  5. Downstream analysis (DE, splicing, pathways)
...
- RNA is first converted into **cDNA fragments**, typically 200–500 bp long.
- **Adapters** are ligated to both ends — these are short, known sequences required for:
...
> 🎯 Goal: produce a high-quality cDNA library compatible with the sequencing platform.

## Common Elements

- **Adapters** — Short, known DNA sequences ligated to fragment ends.  
  Allow fragments to bind to the flow cell surface or nanoball array.

- **Indexes / Barcodes** — Short identifier sequences embedded within adapters.  
  Enable **multiplexing**, allowing several samples to be sequenced in one run.

- **Priming Sites** — Regions within adapters where sequencing primers bind during each read.
...
A 48-replicate yeast study showed that many of the tools available for DGE analysis detected only 20–40% of differentially expressed genes when only 3 replicates were included in the analysis. The study suggested that a minimum of **six biological replicates should be used**, which is substantially more than the three or four replicates generally reported in the RNA-​seq literature.

> ENCODE4: Experiments should have two or more replicates.

## How much sequencing depth?

For bulk RNA DGE experiments in eukaryotic genomes, it is generally accepted that read depths of around 10–30 million reads per sample are required.
...
Paired-end (e.g., 2 × 75 bp or 2 × 100 bp) is preferred for transcript discovery and isoform-level analysis.
...
After alignment, reads are stored in SAM (Sequence Alignment/Map) format — a text-based file describing where each read maps to the reference genome. The BAM format is its binary, compressed version, faster for storage and processing.
...
**1) Transcript Assembly** – Reconstructs full-length transcripts from aligned reads to discover novel isoforms or unannotated genes.
Tools: StringTie, Cufflinks.

**2) RNA Editing Detection** – Identifies base modifications such as A→I or C→U editing events by comparing RNA reads to the reference genome.
Use case: Brain and immune tissues often show strong editing signatures.

**3) Variant Calling from RNA-seq** – Detects SNPs and small indels directly from transcribed regions, revealing expressed genetic variants.
Tools: GATK, bcftools, FreeBayes.

**4) Gene Fusion Detection** – Finds chimeric transcripts produced by genomic rearrangements, common in cancer and developmental disorders.
...
1) Over-representation analysis (ORA) – tests DEGs vs. background.

2) Gene Set Enrichment Analysis (GSEA) – uses ranked gene lists.
...
**Output:** Network modules → correlated with phenotypes or traits.
...
**1) Alternative Splicing Analysis** — detect isoform changes (rMATS, SUPPA2).

**2) Gene Regulatory Network Inference** — identify TF–target relationships (SCENIC, GENIE3).

**3) Integration with Multi-Omics** — combine transcriptomics with proteomics or epigenomics (MOFA, mixOmics).

**4) Visualization Dashboards** — streamline exploration (Shiny, Streamlit, iDEP).
...
She doesn´t want to bother with anything new or out of the shelf, just find basic inflamatory mechanism.
She wants to know:
...
- Research for ~1h for a short-read RNAseq experiment in any research study. Make a two slides presentation (5 minutes or less) in our colective google presentations:
